...
def random_cyclic_plastic_loading_engine_torch(k0: torch.Tensor, kap: torch.Tensor, a: torch.Tensor, c: torch.Tensor,
    ...
    for k_dim in range(2, kap.shape[1], 2):
        kiso = kiso + rep_dir/kap[:, k_dim+1: k_dim+2]*(1-torch.exp(-SQR32 * kap[:, k_dim: k_dim+1] * kap[:, k_dim+1: k_dim+2] * epspc))
    ...
    for r in reversal_ids:
        ...
        in_log = a[:, -2].unsqueeze(-1) / ((a[:, -2] - a[:, -1]).unsqueeze(-1) - alp[:, -1, last_unprocessed: r + 1] * directors[last_unprocessed: r + 1].unsqueeze(0))
        in_log[~if_mask] = 1
        assert torch.all(in_log <= 1)
        depsp_thr = -1. / c[:, -1].unsqueeze(-1) * torch.log(in_log)  # todo: faster
        assert torch.all(depsp_thr >= 0)
        depsp_thr[~if_mask] = 0
